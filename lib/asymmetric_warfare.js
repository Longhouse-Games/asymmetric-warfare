define(['underscore', 'lib/helpers'], function(_, h) {
  ...
  var AsymmetricWarfare = function() {
    ...
    var gameOver = function() {
      ...
      // If only one insurgent remains, they cannot grow or take over the capital
      var totalInsurgents = _.reduce(board.getPositionsByType(h.C.INSURGENT), function(total, position) {
        return total + board.getInsurgentPiecesAt(position).length;
      }, 0);
      if (totalInsurgents <= 1) {
        return cleanup(h.C.STATE);
      }
    ...
    return {
      ...
      state: function(role) {
        ...
        var visibleInsurgents = _.chain(board.getPositionsByType(h.C.INSURGENT))
          .map(function(position) {
            if (gameOver() || board.getInsurgentPiecesAt(position).length >= 2 || board.getStatePiecesAt(position).length >= 1) {
  ...
  var AsymmetricWarfareWithHistory = function(_history) {
    ...
    var forfeit = asymwar.forfeit;
    asymwar.forfeit = function(role) {
      return withHistory(forfeit, role, History.Forfeit)();
    };
