  ...
  function(_, C, Pieces, Position) {
    ...
    var Board = function() {
      ...
      return {
        ...
        getPositionsWithBothTypes: function() {
          var positions = [];
          for (var poskey in pieces) {
            if (pieces.hasOwnProperty(poskey)) {
              var piecesAt = pieces[poskey];
              if (_.find(piecesAt, function(piece) { return piece.type() === Pieces.STATE_TYPE }))
              if (_.find(piecesAt, function(piece) { return piece.type() === Pieces.INSURGENT_TYPE })) {
                positions.push(Position(poskey));
              }
            }
          }
          return positions;
