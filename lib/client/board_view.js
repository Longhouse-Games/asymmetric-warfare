define(['underscore', 'allong.es', 'lib/constants', 'lib/position', 'lib/insurgent_move', 'lib/state_move'],
  ...
  var BoardView = function() {
    ...
    var revealInsurgents = function(positions) {
      _.each(positions, function(position) {
        var $marker = $("#marker"+position.circle+"-"+position.rank).first();
        var $pieces = $("#marker"+position.circle+"-"+position.rank + " .pieces").first();
        // TODO if pieces contains STATE or two insurgents, it's already visible, don't flash
        setTextAndFlashOutline($pieces, $marker, "O!");
      });
    };
