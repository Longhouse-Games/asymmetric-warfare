...
require([
  ...
  var BoardView = function() {
    ...
    var displayInsurgentPlacements = function() {
      ...
      var handler = function(circle, rank) {
        return function() {
          $(".space.circle0").off('click');
          $(".space").removeClass("selectable");
          controller.placeInsurgent(circle, rank);
        }
      }

      for (var i = 0; i < h.C.NUM_RANKS; i++) {
        $findSpace(Position(0)(i)).on('click', handler(0, i));
      }
    };
