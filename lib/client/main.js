  ...
  var BoardView = function() {
    ...
    return {
      ...
      showKillCandidates: function(infowar, controller) {
        clearSelected();
        var positions = _.filter(infowar.getPositionsByType(h.C.STATE), function(position) {
          if (_.find(infowar.getPiecesAt(position), function(piece) { return piece.type() === h.C.STATE; }))
          if (_.find(infowar.getPiecesAt(position), function(piece) { return piece.type() === h.C.INSURGENT; })) {
            //Square contains both a state and an insurgent piece
            return true;
          }
          return false;
        });
      ...
      showGrowCandidates: function(infowar, controller) {
        clearSelected();
        var positions = _.chain(infowar.getPositionsByType(h.C.INSURGENT))
