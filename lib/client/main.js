...
require([
  ...
  var BoardView = function() {
    ...
    var makeSelectable = function(positions, handler) {
      for (var i = 0; i < positions.length; i++) {
        ...
        $findSpace(position).on('click', h(position));
      }
    }

    var displayPossibleMoves = function() {
      $template.on('click', function() {
        var position = Position(circle)(rank);
        var adjacentPositions = position.adjacentPositions();
        $allSpaces().removeClass("short_move");
        $allSpaces().removeClass("long_move");
        $allSpaces().removeClass("selected");
        $(this).addClass("selected");
        for (var i = 0; i < adjacentPositions.length; i++) {
          var distance = position.distanceTo(adjacentPositions[i]);
          var _class;
          if (distance === 1) {
            _class = "short_move";
          } else {
          ...
          $findSpace(adjacentPositions[i]).addClass(_class);
        }
      });
    };
