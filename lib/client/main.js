  ...
  socket.on('connect', function() {
    ...
    socket.on('update', function(data) {
      ...
      if (data.state) {
        ...
        //history to rebuild the infowar item. We can't see the whole board.
        var board = Board();
        ...
        var history = _.map(data.history, function(dto) {
          return dto.message || History.fromDTO(dto);
        });
        historyview.render(history);
        boardview.render(role, board, data.state.currentPhase, data.state.currentTurn, data.state.remainingMovementPoints);
        statusview.render(data.state.currentPhase, data.state.currentTurn);
        actionsview.render(role, board, data.state.currentPhase, data.state.currentTurn);
      } else {
        var history = _.map(data.history, function(dto) {
          return History.fromDTO(dto);
        });
        var infowar = Infowar(history);
        var currentTurn = infowar.currentTurn() ? infowar.currentTurn().id() : null;
        var movementPoints = infowar.currentTurn() ? infowar.currentTurn().movementPoints() : null;
        boardview.render(
            role,
            infowar.getBoard(),
            infowar.currentPhase(),
            currentTurn,
            movementPoints
        );
        actionsview.render(role, infowar.getBoard(), infowar.currentPhase(), currentTurn);
        historyview.render(history);
        statusview.render(infowar.currentPhase(), currentTurn);
      }
