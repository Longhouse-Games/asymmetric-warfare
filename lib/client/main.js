...
require([
  ...
  var BoardView = function() {
    ...
    var makeSelectable = function(positions, handler, deselectAfterClicking, processor) {
      for (var i = 0; i < positions.length; i++) {
        var position = positions[i];
        var h = function(pos) {
          return function() {
            if (deselectAfterClicking) {
              $(".space").off('click');
        ...
        if (processor) {
          processor($space, position);
        }
        $space.addClass('selectable').on('click', h(position));
      }
    }
    ...
    var displayInsurgentPlacements = function() {
      ...
      makeSelectable(positions, function(pos) {
        controller.placeInsurgent(pos.circle, pos.rank);
      }, true);
    };

    var displayMovablePieces = function(player, controllerMethod) {
      $(".space").removeClass("selectable");
      var positions = _.chain(infowar.getPieces())
          .map(function(piecesAt, poskey) {
            if (_.any(piecesAt, function(piece) { return piece.type() === player; })) {
          ...
          .compact()
          .value();
      makeSelectable(positions, function(src) {
        var adjacentPositions = src.adjacentPositions();
        makeSelectable(adjacentPositions,
          function(dest) {
    ...
    var displayMovableInsurgents = function(infowar) {
      displayMovablePieces(h.C.INSURGENT, controller.insurgentMove);
    };

    var displayMovableStatePieces = function(infowar) {
      displayMovablePieces(h.C.STATE, controller.stateMove);
    };
    ...
    return {
      ...
      render: function(infowar) {
        ...
        if (infowar.currentPhase() === h.C.SETUP) {
          ...
          console.log(infowar.currentTurn());
          if (infowar.currentTurn() === h.C.INSURGENT) {
    ...
    return {
      ...
      render: function(infowar) {
        ...
        if (infowar.currentPhase() === h.C.SETUP) {
          ...
          message = infowar.currentTurn() + "'S TURN";
