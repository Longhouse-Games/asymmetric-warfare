define(['lib/constants', 'lib/position'],
  function(C, Position) {
    ...
    var isAdjacent = function(src, dest) {
      ...
      if (dir <= -1) {
        return dest.circle === CAPITAL || src.rank === dest.rank || src.rank === ((dest.rank + 1) % 12);
      }
      if (dir >= 1) {
        return src.circle === CAPITAL || src.rank === dest.rank || src.rank === (((dest.rank - 1)+12) % 12);
      }
      return null;
    };
    ...
    var distance = function(src, dest) {
      if (src.circle !== dest.circle) {
        return 2;
      }
      var diff = Math.abs(src.rank - dest.rank);
      return Math.min(diff, C.NUM_RANKS-diff);
    };
